...
export const SignUpPage = () => {
  ...
  const { mutateAsync: signup } = useMutation({ mutationFn: SignUpApi })
  ...
  const handleSubmitForm = async ({ restaurant, name, email }: SignUpProps) => {
    try {
      await signup({ restaurant, name, email })
      toast.success({
      ...
      <div className={authwrapper()}>
        ...
        <Form className={formcontent()} onSubmit={handleSubmit(handleSubmitForm)}>
          <div className={formgroup()}>
            <input className={forminput()} type='text' id='restaurant' placeholder=' ' {...register('restaurant', { required: { value: true, message: 'Por favor, informe um nome' } })} />
          ...
          <div className={formgroup()}>
            <input className={forminput()} type='text' id='name' placeholder=' ' {...register('name', { required: { value: true, message: 'Por favor, informe um nome' } })} />
          ...
          <button className={formaction({ color: 'primary' })} disabled={!watch('name') || !watch('email')} type='submit'>Cadastrar</button>
        </Form>
