...
const pieData = [{ id: 1, product: 'Pepperoni', amount: 40 }, { id: 2, product: 'Mussarrela', amount: 31 }, { id: 3, product: 'Especial da casa', amount: 24 }, { id: 4, product: 'Quatro queijos', amount: 16 }, { id: 5, product: 'Marguerita', amount: 10 }]
const lineData = [{ data: '01/01', revenue: 1000 }, { data: '02/01', revenue: 1245 }, { data: '03/01', revenue: 3500 }, { data: '04/01', revenue: 2990 }, { data: '05/01', revenue: 4000 }, { data: '06/01', revenue: 1230 }, { data: '07/01', revenue: 1802 }, { data: '08/01', revenue: 2540 }, { data: '09/01', revenue: 890 }, { data: '10/01', revenue: 1300 }]
const dashData = [{ id: 1, title: 'Pedidos', value: '38', descript: '+20.1% em relação ao dia anterior' }, { id: 2, title: 'Concluídos', value: '124', descript: '+18.1% em relação ao mês passado' }, { id: 3, title: 'Cancelados', value: '12', descript: '-8% em relação ao mês passado' }, { id: 4, title: 'Faturamento', value: 'R$ 1448.60', descript: '+38% em relação ao mês passado' }]

export const DashboardPage = () => {
  const labelPieChart = ({ cx, cy, midAngle, innerRadius, outerRadius, value, index }: RechartsProps) => {
              ...
              <ResponsiveContainer width={'100%'} height={350}>
                <PieChart>
                  <Pie cx={'50%'} cy={'50%'} stroke={'#2b2d39'} dataKey={'amount'} nameKey={'product'} strokeWidth={10} labelLine={false} outerRadius={150} innerRadius={115} data={pieData} label={labelPieChart}>
                    {pieData.map((data) => (<Cell key={data.id} fill={pieColors[data.id % pieColors.length]} />))}
                  </Pie>
                </PieChart>
              </ResponsiveContainer>
            </div>
